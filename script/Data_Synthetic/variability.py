...
def generate_noise_series(
    ...
    for _ in range(N):
        ...
        elif noise_type == 'moderate variability':
            if stationary:
                # Generate stationary noise
                series = generate_noise(length=L, noise_std_range=(0.5, 1.0))
            else:
                # Generate non-stationary noise
                series = generate_nonstationary_noise(length=L, noise_std_range=(0.5, 1.0)) # 0.5, 1.0
            description = "The time series exhibits moderate variability."
