...
def load_model(meta, dataset_name, mdl_name, configs_org):
    # config
    ...
    configs['train']['output_folder'] = configs_org['train']['output_folder']#"tedit_save/"+mdl_name+"/"+dataset_name
    print("_"*7, configs['train']['output_folder'], "_"*7)
    # load checkpoint 
...
def tedit_generate(
    ...
    if os.path.exists(model_ckpt):
        state = torch.load(model_ckpt, map_location=device)
        configs["model"]["diffusion"]["diffusion_embedding_dim"] = state["diff_model.diffusion_embedding.projection1.weight"].shape[0]
        model_cfg = configs["model"]
