...
if not os.path.exists(os.path.join(output_dir, 'res_df_iqr.csv')):
    ...
    model, configs = load_model(meta, dataset_name=dataset_name, mdl_name = tedit_mdl, configs_org=configs)
    output_dir = configs['train']['output_folder'] 
    ...
    res_df_msd = pd.read_csv(os.path.join(output_dir, 'res_df_msd.csv'))
    res_df_iqr = pd.read_csv(os.path.join(output_dir, 'res_df_iqr.csv'))