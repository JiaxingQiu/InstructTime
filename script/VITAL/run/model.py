...
if overwrite:  
    ...
    if config_dict['3d']:
        ...
        model = VITAL(
                    ...
                    linear_interpolation = config_dict['linear_interpolation']
                )
    ...
    config_dict = update_config(config_dict, model_name = model_name) 
    ...
    # model = config_dict['model_init'] # change to below
    # print(nn_summary(model))
    # switch linear_interpolation in generation without retraining the model weights
    config_dict['linear_interpolation'] = linear_interpolation if 'linear_interpolation' in locals() else True  # use local/global linear_interpolation if defined, else default to Trueprint('setting linear_interpolation: ', config_dict['linear_interpolation'])
    model = VITAL(
                    ts_dim=ts_f_dim.shape[1],
                    text_dim=tx_f_dim.shape[1],
                    output_dim=config_dict['embedded_dim'],
    ...
    model.linear_interpolation = config_dict['linear_interpolation']
